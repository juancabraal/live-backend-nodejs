...
const loginAttempt = async (attemptee) => {
  const { stream, password, brand } = attemptee;

  const brandSubQuery = Brand.query().where('name', brand);
  const usr = await Brand.relatedQuery('users')
    .for(brandSubQuery)
    .where('login', stream)
    .first();
